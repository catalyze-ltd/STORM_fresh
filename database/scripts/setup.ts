...
scenarioEntities.default.forEach((scenario) => {
    ...
    const iconSrc = './assets/' + scenario["Custom icon"] + '.png';
...
Deno.writeTextFile("database/temp/markers.json", JSON.stringify(markers));
Deno.writeTextFile("database/temp/circles.json", JSON.stringify(circles));
Deno.writeTextFile("database/temp/polylines.json", JSON.stringify(polylines));
Deno.writeTextFile("database/temp/misc.json", JSON.stringify({"allWaypoints": allWaypoints}));

const command = new Deno.Command('python3', {
    args: [ "./database/scripts/setup.py" ],
  });
